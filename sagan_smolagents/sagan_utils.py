...
def save_uploaded_file(file_content: bytes, filename: str, target_dir: Path) -> Path:
    """Save uploaded file content to target directory"""
...
def create_data_vectordb(project_id: str, files: List[tuple]) -> bool:
    """Create vector database for input data files
    ...
    try:
        ...
        for filename, file_content in files:
            try:
                target_path = save_uploaded_file(file_content, filename, data_dir)
...
def create_template_vectordb(project_id: str, template_filename: str, template_content: bytes) -> bool:
    """Create vector database for template file"""
    paths = config_utils.get_project_paths(project_id, config.PROJECTS_BASE)
    ...
    try:
        ...
        target_path = save_uploaded_file(template_content, template_filename, template_dir)
...
def update_vectordb(project_id: str, files: List[tuple]) -> bool:
    """Update vector database with new files.
    files: List of tuples (filename, file_content)"""
    print(f"Updating vector database for project: {project_id}")
    paths = config_utils.get_project_paths(project_id, config.PROJECTS_BASE)
    ...
    try:
        ...
        for filename, file_content in files:
            try:
                ...
                save_uploaded_file(file_content, filename, data_dir)
                processed_files.append(filename)
        ...
        try:
            for filename in processed_files:
                shutil.copy2(data_dir / filename, temp_dir / filename)
